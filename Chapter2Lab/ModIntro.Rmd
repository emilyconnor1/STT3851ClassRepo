...
To display the saved file as shown in Figure \@ref(fig:g1), use the `include_graphics()` function from `knitr`.

```{r, label = "g1", echo = FALSE, fig.cap = "Using knitr::include_graphics()"}
knitr::include_graphics("./JPG/YourFileName.jpeg", dpi=96)
...
## Additional Graphic and Numerical Summaries
...
See the `geom_boxplot` [documentation](http://docs.ggplot2.org/current/geom_boxplot.html) and the `geom_freqpoly` [documentation](http://docs.ggplot2.org/current/geom_boxplot.html) for more details.


```{r, label='moo', fig.cap='documentation'}
```
